...
type ConfigLoader interface {
	Load(context.Context) (*v1.Gateway, error)
	Watch(OnChange)
	Close()
}
...
func (f *FileLoader) initialize() error {
	...
	log.Infof("the initial config files sha256: %s", sha256hex)
	f.priorityConfigHash = pfHash
	log.Infof("the initial priority config files sha256 map: %+v", f.priorityConfigHash)
...
func (f *FileLoader) Load(_ context.Context) (*v1.Gateway, error) {
	log.Infof("loading config files: %s", f.confPath)
...
func (f *FileLoader) mergePriorityConfig(dst *v1.Gateway) error {
	if f.priorityDirectory == "" {
		return nil
	}
	entrys, err := os.ReadDir(f.priorityDirectory)
	...
	for _, e := range entrys {
		...
		if err != nil {
			log.Warnf("failed to parse priority config: %s: %+v, skip merge this files", cfgPath, err)
			continue
		}
...
func (f *FileLoader) parsePriorityConfig(cfgPath string) (*v1.PriorityConfig, error) {
...
func MakeReplaceOrPrependEndpointFn(origin []*v1.Endpoint) func([]*v1.Endpoint, *v1.Endpoint) []*v1.Endpoint {
	keyFn := func(e *v1.Endpoint) string {
	...
	return func(dst []*v1.Endpoint, item *v1.Endpoint) []*v1.Endpoint {
		idx, ok := index[keyFn(item)]
		if !ok {
			return append([]*v1.Endpoint{item}, dst...)
		}
		dst[idx] = item
		return dst
	}
...
func (f *FileLoader) Watch(fn OnChange) {
	log.Info("add config files change event handler")
	f.lock.Lock()
	defer f.lock.Unlock()
...
func (f *FileLoader) watchproc(ctx context.Context) {
	log.Info("start watch config files")
	for {
		select {
		case <-ctx.Done():
			return
		...
		func() {
			sha256hex, pfHash, err := f.configSHA256()
			if err != nil {
				log.Errorf("watch config files error: %+v", err)
				return
			}
			if sha256hex != f.confSHA256 || !reflect.DeepEqual(pfHash, f.priorityConfigHash) {
				log.Infof("config files changed, reload config, last sha256: %s, new sha256: %s, last pfHash: %+v, new pfHash: %+v", f.confSHA256, sha256hex, f.priorityConfigHash, pfHash)
