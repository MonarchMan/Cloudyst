...
func NewSMTPPool(config setting.Provider, logger log.Logger) *SMTPPool {
	client := &SMTPPool{
		config: config.SMTP(context.Background()),
		ch:     make(chan *message, 30),
		chOpen: false,
		l:      log.NewHelper(logger, log.WithMessageKey("biz-email")),
	}
...
func (client *SMTPPool) Send(ctx context.Context, to, title, body string) error {
	...
	client.ch <- &message{
		...
		subject: title,
		to:      to,
		cid:     util.TraceID(ctx),
		userID:  data.UserIDFromContext(ctx),
	}
	return nil
}
...
func (client *SMTPPool) Init() {
	go func() {
		...
		if diaErr != nil {
			client.l.Fatalf("Failed to create SMTP client: %s", diaErr)
			return
		}
		...
		for {
			...
			case m, ok := <-client.ch:
				...
				l := log.NewHelper(log.With(client.l.Logger(), "Cid", m.cid))
				if err := d.Send(m.msg); err != nil {
					...
					l.Warnf("Failed to send email: %s, Cid=%s", err, m.cid)
				} else {
					l.Infof("Email sent to %q, title: %q.", m.to, m.subject)
				}
			// 长时间没有新邮件，则关闭SMTP连接
			case <-time.After(time.Duration(client.config.Keepalive) * time.Second):
				if open {
					if err := d.Close(); err != nil {
						client.l.Warnf("Failed to close SMTP connection: %s", err)
					}
					open = false
				}
			}
