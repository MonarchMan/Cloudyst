...
type (
	Provider interface {
		// SiteBasic returns Site basic information
		...
		CaptchaType(ctx context.Context) string
		...
		// CapCaptcha returns the captcha settings.
		...
		CustomProps(ctx context.Context) []*pbsite.CustomProps
		// CustomNavItems returns the custom nav items settings.
		CustomNavItems(ctx context.Context) []*pbsite.CustomNavItem
		// CustomHTML returns the custom HTML settings.
		CustomHTML(ctx context.Context) *pbsite.CustomHTML
		...
		// EmojiPresets returns the emoji presets used in files icon customization.
		...
		// FileViewers returns the files viewers settings.
		FileViewers(ctx context.Context) []*ftypes.ViewerGroup
...
func (s *settingProvider) ForgotPasswordCaptchaEnabled(ctx context.Context) bool {
	return s.getBoolean(ctx, "forgot_captcha", false)
}
...
func (s *settingProvider) CaptchaType(ctx context.Context) string {
	return s.getString(ctx, "captcha_type", string(CaptchaNormal))
}
...
func (s *settingProvider) Captcha(ctx context.Context) *Captcha {
	return &Captcha{
		...
		Mode:               CaptchaMode(s.getInt(ctx, "captcha_mode", int(CaptchaModeNumberString))),
		ComplexOfNoiseText: s.getInt(ctx, "captcha_ComplexOfNoiseText", 0),
		MaxSkew:            s.getFloat64(ctx, "captcha_ComplexOfNoiseDot", 0),
		Fonts:              s.getStringList(ctx, "captcha_fonts", []string{}),
		BgColor:            s.getBoolean(ctx, "captcha_IsShowNoiseDot", false),
		IsShowNoiseText:    s.getBoolean(ctx, "captcha_IsShowNoiseText", false),
		Source:             s.getString(ctx, "captcha_Source", ""),
		ShowLineOptions:    s.getInt(ctx, "captcha_ShowSineLine", 0),
		Length:             s.getInt(ctx, "captcha_CaptchaLen", 6),
		Language:           s.getString(ctx, "captcha_language", "zh"),
	}
}
...
func (s *settingProvider) CustomProps(ctx context.Context) []*pbsite.CustomProps {
	raw := s.getString(ctx, "custom_props", "[]")
	var props []*pbsite.CustomProps
	if err := json.Unmarshal([]byte(raw), &props); err != nil {
		return []*pbsite.CustomProps{}
	}
	return props
}

func (s *settingProvider) CustomNavItems(ctx context.Context) []*pbsite.CustomNavItem {
	raw := s.getString(ctx, "custom_nav_items", "[]")
	var items []*pbsite.CustomNavItem
	if err := json.Unmarshal([]byte(raw), &items); err != nil {
		return []*pbsite.CustomNavItem{}
	}
	return items
}

func (s *settingProvider) CustomHTML(ctx context.Context) *pbsite.CustomHTML {
	return &pbsite.CustomHTML{
...
func (s *settingProvider) FileViewers(ctx context.Context) []*ftypes.ViewerGroup {
	raw := s.getString(ctx, "file_viewers", "[]")
	var viewers []*ftypes.ViewerGroup
	if err := json.Unmarshal([]byte(raw), &viewers); err != nil {
		return []*ftypes.ViewerGroup{}
	}
...
func (s *settingProvider) MapSetting(ctx context.Context) *MapSetting {
	return &MapSetting{
		Provider:       s.getString(ctx, "map_provider", "openstreetmap"),
		GoogleTileType: s.getString(ctx, "map_google_tile_type", "roadmap"),
	}
}
