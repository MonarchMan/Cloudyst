...
var (
	ErrUserEmailExisted      = errors.New("users email has been registered")
...
type (
	UserClient interface {
		TxOperator
		// New creates a new users. If users email registered, existed User will be returned.
		Create(ctx context.Context, args *NewUserArgs) (*ent.User, error)
		// GetByEmail get the users with given email, users status is ignored.
		GetByEmail(ctx context.Context, email string) (*ent.User, error)
		// GetByID get users by its ID, users status is ignored.
		GetByID(ctx context.Context, id int) (*ent.User, error)
		// GetActiveByID get users by its ID, only active users will be returned.
		GetActiveByID(ctx context.Context, id int) (*ent.User, error)
		// SetStatus Set users to given status
		SetStatus(ctx context.Context, u *ent.User, status user.Status) (*ent.User, error)
		// AnonymousUser returns the anonymous users.
		...
		// ApplyStorageDiff apply storage diff to users.
		...
		// UpdateNickname updates users nickname.
		...
		// CountByTimeRange count users by time range. Will return all records if start or end is empty.
		...
		// BatchDelete deletes users by its ID.
		BatchDelete(ctx context.Context, uids []int) (int, error)
	}
...
type (
	// NewUserArgs args to create a new users
	NewUserArgs struct {
		Email         string
...
func (c *userClient) BatchDelete(ctx context.Context, uids []int) (int, error) {
	// Dav accounts
	if num, err := c.client.DavAccount.Delete().Where(davaccount.OwnerIDIn(uids...)).Exec(schema.SkipSoftDelete(ctx)); err != nil {
		return num, fmt.Errorf("failed to delete dav accounts: %w", err)
	}
	// Passkeys
	if num, err := c.client.Passkey.Delete().Where(passkey.UserIDIn(uids...)).Exec(schema.SkipSoftDelete(ctx)); err != nil {
		return num, fmt.Errorf("failed to delete passkeys: %w", err)
	}
	// Tasks

	return c.client.User.Delete().Where(user.IDIn(uids...)).Exec(schema.SkipSoftDelete(ctx))
}
	...
	for uid, diff := range diffs {
		...
		for attempt := 0; attempt < maxRetries; attempt++ {
			if err := c.client.User.Update().Where(user.ID(uid)).AddStorage(diff).Exec(ctx); err != nil {
				...
				ae.Add(fmt.Sprintf("%d", uid), fmt.Errorf("failed to apply storage diff for users %d: %w", uid, err))
				break
			}
		...
		if lastErr != nil {
			ae.Add(fmt.Sprintf("%d", uid), fmt.Errorf("failed to apply storage diff for users %d: %w", uid, lastErr))
		}
	}
...
func (c *userClient) Create(ctx context.Context, args *NewUserArgs) (*ent.User, error) {
	// Try to check if there's users with same email.
	...
	if newUser.ID == 1 {
		// For the first users registered, elevate it to admin group.
		if _, err := newUser.Update().SetGroupID(1).Save(ctx); err != nil {
			return newUser, fmt.Errorf("failed to elevate users to admin: %w", err)
		}
	}
	return newUser, nil
}
...
func (c *userClient) GetLoginUserByID(ctx context.Context, uid int) (*ent.User, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to construct anonymous users: %w", err)
	}
...
// UserFromContext get users from context
func UserFromContext(ctx context.Context) *ent.User {
	u, _ := ctx.Value(UserCtx{}).(*ent.User)
	return u
}

// UserIDFromContext get users id from context.
...
// IsAnonymousUser check if given users is anonymous users.
