...
func (c *groupClient) AnonymousGroup(ctx context.Context) (*ent.Group, error) {
	return c.client.Group.Query().Where(group.ID(AnonymousGroupID)).First(ctx)
}

func (c *groupClient) ListAll(ctx context.Context) ([]*ent.Group, error) {
	return c.client.Group.Query().All(ctx)
}
...
func (c *groupClient) Upsert(ctx context.Context, group *ent.Group) (*ent.Group, error) {
	if group.ID == 0 {
		...
		if group.StoragePolicyID > 0 {
			...
			if group.StoragePolicyInfo != nil {
				stm.SetStoragePolicyInfo(group.StoragePolicyInfo)
			}
		}
		return stm.Save(ctx)
	}
...
func (c *groupClient) GetByID(ctx context.Context, id int) (*ent.Group, error) {
	return c.client.Group.Query().Where(group.ID(id)).First(ctx)
}
