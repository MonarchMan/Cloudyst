...
func (_q *UserQuery) QueryPasskey() *PasskeyQuery {
...
func (_q *UserQuery) QueryDavAccounts() *DavAccountQuery {
	query := (&DavAccountClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withDavAccounts: _q.withDavAccounts.Clone(),
...
func (_q *UserQuery) WithPasskey(opts ...func(*PasskeyQuery)) *UserQuery {
	query := (&PasskeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithDavAccounts(opts ...func(*DavAccountQuery)) *UserQuery {
	query := (&DavAccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDavAccounts = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withPasskey; query != nil {
		if err := _q.loadPasskey(ctx, query, nodes,
			func(n *User) { n.Edges.Passkey = []*Passkey{} },
			func(n *User, e *Passkey) { n.Edges.Passkey = append(n.Edges.Passkey, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDavAccounts; query != nil {
		if err := _q.loadDavAccounts(ctx, query, nodes,
...
func (_q *UserQuery) loadPasskey(ctx context.Context, query *PasskeyQuery, nodes []*User, init func(*User), assign func(*User, *Passkey)) error {
...
func (_q *UserQuery) loadDavAccounts(ctx context.Context, query *DavAccountQuery, nodes []*User, init func(*User), assign func(*User, *DavAccount)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
