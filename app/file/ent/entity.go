	...
	for i := range columns {
		...
		case entity.FieldStoragePolicyEntities:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				_m.StoragePolicyEntities = int(value.Int64)
			}
...
func (_m *Entity) QueryFile() *FileQuery {
	return NewEntityClient(_m.config).QueryFile(_m)
}

// QueryStoragePolicy queries the "storage_policy" edge of the Entity entity.
func (_m *Entity) QueryStoragePolicy() *StoragePolicyQuery {
	return NewEntityClient(_m.config).QueryStoragePolicy(_m)
}
...
func (_m *Entity) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", _m.StoragePolicyEntities))
	builder.WriteString(", ")
