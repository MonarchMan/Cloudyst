...
type (
	UserSetting struct {
		...
		FsViewMap           map[string]pb.ExplorerView `json:"fs_view_map,omitempty"`
	...
	PolicySetting struct {
		...
		// CustomProxy whether to use custom-proxy to get files content
		CustomProxy bool `json:"custom_proxy,omitempty"`
		// ProxyServer 反代地址
		ProxyServer string `json:"proxy_server,omitempty"`
		// InternalProxy whether to use Cloudreve external proxy to get files content
		...
		// Whether to support all files extensions for thumbnail generation.
		...
		// Whether to upload files through server's relay.
		Relay bool `json:"relay,omitempty"`
		// Whether to pre allocate space for files before upload in physical disk.
		PreAllocate bool `json:"pre_allocate,omitempty"`
		// MediaMetaExts files extensions that support media meta generation using native policy API.
		...
		// StreamSaver whether to use stream saver to download files in Web.
	...
	QBittorrentSetting struct {
		...
		User     string         `json:"users,omitempty"`
	...
	ShareProps struct {
		...
		// Whether to automatically show readme files in share view
...
const (
	EntityTypeVersion = iota
	EntityTypeThumbnail
	EntityTypeLivePhoto
)

func FileTypeFromString(s string) int {
	switch s {
	case "files":
		return FileTypeFile
	case "folder":
