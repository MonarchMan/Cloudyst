...
func (c *fileClient) searchQuery(q *ent.FileQuery, args *SearchFileParameters, parents []*ent.File, ownerId int) *ent.FileQuery {
...
// ChildFileQuery generates query for child files(s) of a given set of root
func (c *fileClient) childFileQuery(ownerID int, isSymbolic bool, root ...*ent.File) *ent.FileQuery {
	rawQuery := c.client.File.Query()
	if len(root) == 1 && root[0] != nil {
		// Query children of one single root
		rawQuery = c.client.File.QueryChildren(root[0])
	} else if root[0] == nil {
		// Query orphan files with OwnerId ID
		...
		if isSymbolic {
			predicates = append(predicates, file.And(file.IsSymbolic(true), file.FileParentIDIsNil()))
		} else {
			predicates = append(predicates, file.Not(file.HasParent()))
		}

		rawQuery = c.client.File.Query().Where(
			file.And(predicates...),
		)
	} else {
...
func (c *fileClient) batchInCondition(pageSize, margin int, multiply int, ids []int) ([]predicate.File, [][]int) {
	pageSize = capPageSize(c.maxSQlParam, pageSize, margin)
...
func (c *fileClient) batchInConditionMetadataName(pageSize, margin int, multiply int, keys []string) ([]predicate.Metadata, [][]string) {
	pageSize = capPageSize(c.maxSQlParam, pageSize, margin)
...
func (c *fileClient) batchInConditionEntityID(pageSize, margin int, multiply int, keys []int) ([]predicate.Entity, [][]int) {
	pageSize = capPageSize(c.maxSQlParam, pageSize, margin)
	chunks := lo.Chunk(keys, max(pageSize/multiply, 1))
...
func (c *fileClient) cursorPagination(ctx context.Context, query *ent.FileQuery,
	args *ListFileParameters, paramMargin int) ([]*ent.File, *pb.PaginationResults, error) {
	...
	if args.PageToken != "" {
		pageToken, err = pageTokenFromString(args.PageToken, c.hasher, hashid.FileID)
		if err != nil {
	...
	if len(files) > pageSize {
		...
		nextToken, err := getFileNextPageToken(c.hasher, lastItem, args, nextStartWithFile)
		if err != nil {
	...
	return lo.Subset(files, 0, uint(pageSize)), &pb.PaginationResults{
...
func (c *fileClient) offsetPagination(ctx context.Context, query *ent.FileQuery,
	args *ListFileParameters, paramMargin int) ([]*ent.File, *pb.PaginationResults, error) {
	pageSize := capPageSize(c.maxSQlParam, args.PageSize, paramMargin)
	...
	return allFiles, &pb.PaginationResults{
		TotalItems: int32(folderCount + fileCount),
...
// getFileNextPageToken returns the next page token for the given last files.
