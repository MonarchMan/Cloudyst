...
func needMigration(client *ent.Client, ctx context.Context, requiredDbVersion string) bool {
	c, _ := client.Setting.Query().Where(setting.NameEQ(DBVersionPrefix + ServicePrefix + requiredDbVersion)).Count(ctx)
	return c == 0
}
...
func migrate(l *log.Helper, client *ent.Client, ctx context.Context, kv cache.Driver, requiredDbVersion string) error {
	...
	client.Setting.Create().SetName(DBVersionPrefix + ServicePrefix + requiredDbVersion).SetValue("installed").Save(ctx)
	return nil
}

func migrateDefaultStoragePolicy(l *log.Helper, client *ent.Client, ctx context.Context) error {
...
func migrateMasterNode(l *log.Helper, client *ent.Client, ctx context.Context) error {
	...
	stm := client.Node.Create().
		...
		SetName("Master").
		SetSettings(&pb.NodeSetting{
			Provider: pb.DownloaderProvider_DOWNLOADER_PROVIDER_ARIA2,
		}).
	...
	if enableAria2 {
		...
		stm.SetSettings(&pb.NodeSetting{
			Provider: pb.DownloaderProvider_DOWNLOADER_PROVIDER_ARIA2,
			Aria2: &pb.Aria2Setting{
...
var patches = []Patch{
	...
	},
	{
		...
		EndVersion: "4.1.0",
		Func: func(l *log.Helper, client *ent.Client, ctx context.Context) error {
			// 1. Apply excalidraw files icons
...
func applyPatches(l *log.Helper, client *ent.Client, ctx context.Context, requiredDbVersion string) error {
	...
	for _, v := range allVersionMarks {
		...
		if err != nil {
			l.WithContext(ctx).Warnf("Failed to parse past version %s: %s", v.Name, err)
			continue
		}
