...
var (
	...
	ErrSourceFileInvalid = fmt.Errorf("source files is deleted")
)
...
type (
	ShareClient interface {
		...
		// GetByIDUser returns the share with given id and users id.
...
func (c *shareClient) GetByIDUser(ctx context.Context, id, uid int) (*ent.Share, error) {
	s, err := withShareEagerLoading(ctx, c.client.Share.Query().
		Where(share.ID(id))).
		Where(share.OwnerID(uid)).First(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to query share %d: %w", id, err)
	}
...
func (c *shareClient) DeleteBatchByUserID(ctx context.Context, uid int, shareIds []int) error {
	_, err := c.client.Share.Delete().Where(share.IDIn(shareIds...)).Where(share.OwnerID(uid)).Exec(ctx)
	return err
}
...
func IsValidShare(share *ent.Share) error {
	...
	// Check owner status
	owner := share.OwnerInfo
	if owner == nil || owner.Status != constants.StatusActive {
		// OwnerId already deleted, or not active.
		return ErrOwnerInactive
	}

	// Check source files status
	file, err := share.Edges.FileOrErr()
	if err != nil || file.FileParentID == 0 || file.OwnerID != share.OwnerID {
		// Source files already deleted
...
func (c *shareClient) cursorPagination(ctx context.Context, query *ent.ShareQuery, args *ListShareArgs,
	paramMargin int) ([]*ent.Share, *pb.PaginationResults, error) {
	...
	return lo.Subset(logs, 0, uint(pageSize)), &pb.PaginationResults{
...
func (c *shareClient) offsetPagination(ctx context.Context, query *ent.ShareQuery, args *ListShareArgs,
	paramMargin int) ([]*ent.Share, *pb.PaginationResults, error) {
	pageSize := capPageSize(c.maxSQlParam, args.PageSize, paramMargin)
	query.Order(getShareOrderOption(args)...)
...
func (c *shareClient) listQuery(args *ListShareArgs) *ent.ShareQuery {
	...
	if args.UserID > 0 {
		query.Where(share.OwnerID(args.UserID))
	}
