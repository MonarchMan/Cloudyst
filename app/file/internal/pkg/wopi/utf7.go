...
// copy from golang.org/x/text/encoding/external
	...
	for ; nSrc < n; nSrc++ {
		...
		if tmp == start {
			if tmp == n {
				...
				// '+' can not at the end, so get more constants
				return nDst, nSrc, transform.ErrShortSrc
			}
			dst[nDst] = '+' // Escape sequence "+-"
			nDst++
		} else if tmp == n && !atEOF {
			// no end of BASE64 marker and still has constants
			// probably the marker at next block of constants
			// so go get more constants.
			return nDst, nSrc, transform.ErrShortSrc
...
// utf7dec extracts UTF-16-BE bytes from Base64 constants and converts them to UTF-8.
// A nil slice is returned if the encoding is invalid.
func utf7dec(b64 []byte) []byte {
	var b []byte

	// Allocate a single block of memory large enough to store the Base64 constants
