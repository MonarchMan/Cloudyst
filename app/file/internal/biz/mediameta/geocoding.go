...
func newGeocodingExtractor(settings setting.Provider, l log.Logger, client request.Client) *geocodingExtractor {
	return &geocodingExtractor{
		settings: settings,
		l:        log.NewHelper(l, log.WithMessageKey("biz-mediameta")),
		client:   client,
	}
}
