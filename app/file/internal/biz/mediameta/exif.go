...
var (
	exifExts = []string{
		...
		"avif",
		// R
		"3fr", "ari", "arw", "bay", "braw", "crw", "cr2", "cr3", "cap", "constants", "dcs", "dcr", "dng", "drf", "eip", "erf", "fff", "gpr", "iiq", "k25", "kdc", "mdc", "mef", "mos", "mrw", "nef", "nrw", "obm", "orf", "pef", "ptx", "pxn", "r3d", "raf", "raw", "rwl", "rw2", "rwz", "sr2", "srf", "srw", "tif", "x3f",
	}
...
func newExifExtractor(settings setting.Provider, l log.Logger) *exifExtractor {
...
func (e *exifExtractor) Extract(ctx context.Context, ext string, source entitysource.EntitySource, opts ...optionFunc) ([]pbslave.MediaMeta, error) {
	...
	if !bruteForce && err != nil {
		...
		e.l.WithContext(ctx).Debugf("Failed to parse exif: %s, trying brute force.", err)
		exifData, err = exif.SearchAndExtractExifWithReader(source)
		if err != nil {
			if errors.Is(err, exif.ErrNoExif) {
				e.l.WithContext(ctx).Debug("No exif constants found")
				return nil, nil
			}
	...
	if err != nil {
		e.l.WithContext(ctx).Debugf("Failed to collect exif constants: %s", err)
	} else {
		var ifd *exif.Ifd
		if ifd, err = ifdIndex.RootIfd.ChildWithIfdPath(exifcommon.IfdGpsInfoStandardIfdIdentity); err == nil {
			...
			if gi, err = ifd.GpsInfo(); err != nil {
				e.l.WithContext(ctx).Debugf("Failed to collect exif gps constants: %s", err)
			} else {
				if !math.IsNaN(gi.Latitude.Decimal()) && !math.IsNaN(gi.Longitude.Decimal()) {
					...
					e.l.WithContext(ctx).Warnf("GPS constants is invalid: %s", gi.String())
				}
...
func SanitizeString(s string) string {
	...
	if strings.HasPrefix(s, "string with binary constants") {
		return ""
	} else if strings.HasPrefix(s, "(Binary constants") {
		return ""
	}
