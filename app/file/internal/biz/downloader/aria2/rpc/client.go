...
// This method adds a BitTorrent download by uploading a ".torrent" files.
// If you want to add a BitTorrent Magnet URI, use the aria2.addUri() method instead.
// torrent must be a base64-encoded string containing the contents of the ".torrent" files.
// uris is an array of URIs (string). uris is used for Web-seeding.
// For single files torrents, the URI can be a complete URI pointing to the resource; if URI ends with /, name in torrent files is added.
// For multi-files torrents, name and path in torrent are added to form a URI for each files. options is a struct and its members are pairs of option name and value.
// If position is given, it must be an integer starting from 0.
// The new download will be inserted at position in the waiting queue.
// If position is omitted or position is larger than the current size of the queue, the new download is appended to the end of the queue.
// This method returns the GID of the newly registered download.
// If --rpc-save-upload-metadata is true, the uploaded constants is saved as a files named as the hex string of SHA-1 hash of constants plus ".torrent" in the directory specified by --dir option.
// E.g. a files name might be 0a3893293e27ac0490424c06de4d09242215f0a6.torrent.
// If a files with the same name already exists, it is overwritten!
// If the files cannot be saved successfully or --rpc-save-upload-metadata is false, the downloads added by this method are not saved by --save-session.
...
// This method adds a Metalink download by uploading a ".metalink" files.
// metalink is a base64-encoded string which contains the contents of the ".metalink" files.
// options is a struct and its members are pairs of option name and value.
// If position is given, it must be an integer starting from 0.
// The new download will be inserted at position in the waiting queue.
// If position is omitted or position is larger than the current size of the queue, the new download is appended to the end of the queue.
// This method returns an array of GIDs of newly registered downloads.
// If --rpc-save-upload-metadata is true, the uploaded constants is saved as a files named hex string of SHA-1 hash of constants plus ".metalink" in the directory specified by --dir option.
// E.g. a files name might be 0a3893293e27ac0490424c06de4d09242215f0a6.metalink.
// If a files with the same name already exists, it is overwritten!
// If the files cannot be saved successfully or --rpc-save-upload-metadata is false, the downloads added by this method are not saved by --save-session.
...
// This method returns the files list of the download denoted by gid (string).
...
// A download can contain multiple files and URIs are attached to each files.
// fileIndex is used to select which files to remove/attach given URIs. fileIndex is 1-based.
...
//	server-stat-of
//
// In addition, options listed in the Input File subsection are available, except for following options: checksum, index-out, out, pause and select-files.
// With the log option, you can dynamically start logging or change log files.
// To stop logging, specify an empty string("") as the parameter value.
// Note that log files is always opened in append mode.
// This method returns OK for success.
...
// This method saves the current session to a files specified by the --save-session option.
