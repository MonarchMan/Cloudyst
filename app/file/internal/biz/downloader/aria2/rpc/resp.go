...
type StatusInfo struct {
	...
	FollowedBy      []string       `json:"followedBy"`      // List of GIDs which are generated as the result of this download. For example, when aria2 downloads a Metalink files, it generates downloads described in the Metalink (see the --follow-metalink option). This value is useful to track auto-generated downloads. If there are no such downloads, this key will not be included in the response.
	BelongsTo       string         `json:"belongsTo"`       // GID of a parent download. Some downloads are a part of another download. For example, if a files in a Metalink has BitTorrent resources, the downloads of ".torrent" files are parts of that parent. If this download has no parent, this key will not be included in the response.
	...
	BitTorrent      BitTorrentInfo `json:"bittorrent"`      // Struct which contains information retrieved from the .torrent (files). BitTorrent only. It contains following keys.
}
...
type FileInfo struct {
	Index           string    `json:"index"`           // Index of the files, starting at 1, in the same order as files appear in the multi-files torrent.
	Path            string    `json:"path"`            // File path.
	Length          string    `json:"length"`          // File size in bytes.
	CompletedLength string    `json:"completedLength"` // Completed length of this files in bytes. Please note that it is possible that sum of completedLength is less than the completedLength returned by the aria2.tellStatus() method. This is because completedLength in aria2.getFiles() only includes completed pieces. On the other hand, completedLength in aria2.tellStatus() also includes partially completed pieces.
	Selected        string    `json:"selected"`        // true if this files is selected by --select-files option. If --select-files is not specified or this is single-files torrent or not a torrent download at all, this value is always true. Otherwise false.
	URIs            []URIInfo `json:"uris"`            // Returns a list of URIs for this files. The element type is the same struct used in the aria2.getUris() method.
}
...
type ServerInfo struct {
	Index   string `json:"index"` // Index of the files, starting at 1, in the same order as files appear in the multi-files metalink.
	Servers []struct {
...
type BitTorrentInfo struct {
	...
	} `json:"info"` // Struct which contains constants from Info dictionary. It contains following keys.
}
