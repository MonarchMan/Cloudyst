...
type qbittorrentClient struct {
	...
	l        *log.Helper
	options  *pb.QBittorrentSetting
}

func NewClient(l log.Logger, c request.Client, setting setting.Provider, options *pb.QBittorrentSetting) (downloader.Downloader, error) {
	...
	return &qbittorrentClient{c: c, options: options, l: log.NewHelper(l, log.WithMessageKey("qbittorrent")), settings: setting}, nil
}

func (c *qbittorrentClient) SetFilesToDownload(ctx context.Context, handle *downloader.TaskHandle, args ...*pbexplorer.SetFileToDownloadArgs) error {
	downloadId := make([]int, 0, len(args))
	skipId := make([]int, 0, len(args))
	for _, arg := range args {
		if arg.Download {
			downloadId = append(downloadId, int(arg.Index))
		} else {
			skipId = append(skipId, int(arg.Index))
		}
	}

	if len(downloadId) > 0 {
		if err := c.setFilePriority(ctx, handle.Hash, downloadPriorityDownload, downloadId...); err != nil {
			return fmt.Errorf("failed to set files priority to download: %w", err)
		}
	}

	if len(skipId) > 0 {
		if err := c.setFilePriority(ctx, handle.Hash, downloadPrioritySkip, skipId...); err != nil {
			return fmt.Errorf("failed to set files priority to skip: %w", err)
		}
	}
...
func (c *qbittorrentClient) CreateTask(ctx context.Context, url string, options map[string]interface{}) (*downloader.TaskHandle, error) {
	...
	c.l.WithContext(ctx).Infof("Creating QBitTorrent task with url %q saving to %q...", url, path)
	...
	for k, v := range c.options.Options.AsMap() {
...
func (c *qbittorrentClient) setFilePriority(ctx context.Context, hash string, priority int, id ...int) error {
	...
	if err != nil {
		return fmt.Errorf("failed to set files priority: %w", err)
	}
...
func (c *qbittorrentClient) request(ctx context.Context, method, path string, body string, headers *http.Header) (string, error) {
	...
	case http.StatusForbidden:
		c.l.WithContext(ctx).Info("QBittorrent cookie expired, sending login request...")
	...
	case http.StatusUnsupportedMediaType:
		return "", fmt.Errorf("invalid torrent files")
	default:
