...
func NewChunkGroup(file *fs.UploadRequest, chunkSize int64, backoff backoff.Backoff, useBuffer bool, l *log.Helper, tempPath string) *ChunkGroup {
	c := &ChunkGroup{
		file:              file,
		chunkSize:         chunkSize,
		backoff:           backoff,
...
// TempAvailable returns if current chunk temp files is available to be read
...
func (c *ChunkGroup) Process(processor ChunkProcessFunc) error {
	...
	// If useBuffer is enabled, tee the reader to a temp files
	if c.enableRetryBuffer && c.bufferTemp == nil && !c.file.Seekable() {
		...
		if err != nil {
			c.l.Warnf("Failed to create temp chunk buffer files: %s", err)
		}
	...
	if c.bufferTemp != nil {
		...
		// if temp buffer files is available, use it
		if c.TempAvailable() {
			if _, err := c.bufferTemp.Seek(0, io.SeekStart); err != nil {
				return fmt.Errorf("failed to seek temp files back to chunk start: %w", err)
			}

			c.l.Debugf("Chunk %d will be read from temp files %q.", c.Index(), c.bufferTemp.Name())
	...
	if err != nil {
		...
		if err != context.Canceled && (c.file.Seekable() || c.TempAvailable()) && c.backoff.Next(err) {
			...
			c.l.Debugf("Retrying chunk %d, last error: %s", c.currentIndex, err)
	...
	c.l.Debugf("Chunk %d processed", c.currentIndex)
	return nil
}
