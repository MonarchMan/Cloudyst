...
func New(ctx context.Context, policy *ent.StoragePolicy, settings setting.Provider,
	config *conf.Bootstrap, l *log.Helper) (*Driver, error) {
...
func (handler *Driver) Thumb(ctx context.Context, expire *time.Time, ext string, e fs.Entity) (string, error) {
	...
	signedThumbURL, err := auth.SignURI(handler.AuthInstance, thumbURL.String(), expire)
	if err != nil {
		return "", err
	}
...
func (handler *Driver) Source(ctx context.Context, e fs.Entity, args *driver.GetSourceArgs) (string, error) {
	...
	nodeId := 0
	if handler.config.Server.Sys.Mode == constants.SlaveMode {
	...
	internalProxyed, err := auth.SignURI(handler.AuthInstance, base.String(), args.Expire)
	if err != nil {
		return "", fmt.Errorf("failed to sign external slave content URL: %w", err)
	}
