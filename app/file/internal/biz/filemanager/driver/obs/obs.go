...
func (d *Driver) List(ctx context.Context, base string, onProgress driver.ListProgressFunc, recursive bool) ([]fs.PhysicalObject, error) {
	...
	for {
		...
		if err != nil {
			d.l.WithContext(ctx).Warnf("Failed to list objects: %s", err)
			return nil, err
		}
	...
	for _, object := range commons {
		...
		if err != nil {
			d.l.WithContext(ctx).Warnf("Failed to get relative path: %s", err)
			continue
		}
...
func (d *Driver) Delete(ctx context.Context, files ...string) ([]string, error) {
	...
	for index, group := range groups {
		d.l.WithContext(ctx).Debugf("Process delete group #%d: %v", index, group)
		// 删除文件
		...
		for _, v := range delRes.Errors {
			d.l.WithContext(ctx).Debugf("Failed to delete files: %s, Code:%s, Message:%s", v.Key, v.Code, v.Key)
...
func (d *Driver) cancelUpload(path string, imur *obs.InitiateMultipartUploadOutput) {
	if _, err := d.obs.AbortMultipartUpload(&obs.AbortMultipartUploadInput{
		...
		d.l.Warnf("failed to abort multipart upload: %s", err)
	}
}
