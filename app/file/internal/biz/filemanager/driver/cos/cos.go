...
func New(ctx context.Context, policy *ent.StoragePolicy, settings setting.Provider,
	config *conf.Bootstrap, l log.Logger, mime mime.MimeDetector) (*Driver, error) {
...
func (handler *Driver) List(ctx context.Context, base string, onProgress driver.ListProgressFunc, recursive bool) ([]fs.PhysicalObject, error) {
	...
	for {
		...
		if err != nil {
			handler.l.WithContext(ctx).Warnf("Failed to list objects: %s", err)
			return nil, err
		}
...
func (handler Driver) Delete(ctx context.Context, files ...string) ([]string, error) {
	...
	for index, group := range groups {
		handler.l.WithContext(ctx).Debugf("Process delete group #%d: %v", index, group)
		...
		for _, v := range res.Errors {
			handler.l.WithContext(ctx).Debugf("Failed to delete files: %s, Code:%s, Message:%s", v.Key, v.Code, v.Key)
...
func (handler *Driver) cancelUpload(path, uploadId string) {
	if _, err := handler.client.Object.AbortMultipartUpload(context.Background(), path, uploadId); err != nil {
		handler.l.Warnf("failed to abort multipart upload: %s", err)
	}
}
