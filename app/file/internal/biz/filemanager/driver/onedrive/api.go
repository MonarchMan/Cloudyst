	...
	for {
		...
		if err != nil {
			...
			if retried < ListRetry {
				retried++
				client.l.WithContext(ctx).Debugf("Failed to list path %q: %s, will retry in 5 seconds.", path, err)
		...
		client.l.WithContext(ctx).Debugf("Load next page, next link: %s", fileInfo.NextLink)
	...
	for chunks.Next() {
		if err := chunks.Process(uploadFunc); err != nil {
			if err := client.DeleteUploadSession(ctx, uploadURL); err != nil {
				client.l.WithContext(ctx).Warnf("Failed to delete upload session: %s", err)
			}
...
func (client *client) BatchDelete(ctx context.Context, dst []string) ([]string, error) {
	...
	for i := 0; i < groupNum; i++ {
		...
		client.l.WithContext(ctx).Debugf("Delete files group: %v.", dst[20*i:end])
...
func (client *client) request(ctx context.Context, method string, url string, body io.Reader, option ...request.Option) (string, error) {
	...
	if res.Response.StatusCode < 200 || res.Response.StatusCode >= 300 {
		...
		if decodeErr != nil {
			client.l.WithContext(ctx).Debugf("Onedrive returns unknown response: %s", respBody)
			return "", sysError(decodeErr)
		}

		if res.Response.StatusCode == 429 {
			client.l.WithContext(ctx).Warnf("OneDrive request is throttled.")
