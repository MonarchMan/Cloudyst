...
func (c Credential) Refresh(ctx context.Context, pc data.StoragePolicyClient, config *conf.Bootstrap, l *log.Helper) (
	credmanager.Credential, error) {
	if c.RefreshToken == "" {
		return nil, ErrInvalidRefreshToken
	}

	policy, err := pc.GetPolicyByID(ctx, c.PolicyID)
	if err != nil {
	...
	newCredential, err := obtainToken(ctx, &obtainTokenArgs{
		...
		client:        request.NewClient(config.Server.Sys.Mode, request.WithLogger(l.Logger())),
	...
	if err := pc.UpdateAccessKey(ctx, policy, newCredential.RefreshToken); err != nil {
		return nil, err
	}
...
func RetrieveOneDriveCredentials(ctx context.Context, storagePolicyClient data.StoragePolicyClient) ([]credmanager.Credential, error) {
