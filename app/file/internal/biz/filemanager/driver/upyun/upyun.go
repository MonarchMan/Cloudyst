...
type (
	...
	UploadPolicy struct {
		...
		AllowFileType      string `json:"allow-files-type,omitempty"`
	}
	// Driver 又拍云策略适配器
	Driver struct {
...
func New(ctx context.Context, policy *ent.StoragePolicy, settings setting.Provider,
	...
	driver := &Driver{
		...
		settings:   settings,
		config:     config,
		l:          log.NewHelper(l, log.WithMessageKey("biz-dirver")),
		mime:       mime,
...
func ValidateCallback(req *http.Request, session *fs.UploadSession) error {
	...
	actualSignature := req.Header.Get("Authorization")
