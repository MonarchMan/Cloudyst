...
func NewUriFromString(u string) (*URI, error) {
	...
	if raw.Scheme != constants.CloudScheme && raw.Scheme != "cloudreve" {
...
func (u *URI) SearchParameters() *data.SearchFileParameters {
	q := u.U.Query()
	res := &data.SearchFileParameters{
		Metadata: make([]data.MetadataFilter, 0),
	}
	withSearch := false
	...
	for k, v := range q {
		if strings.HasPrefix(k, QuerySearchMetadataPrefix) {
			res.Metadata = append(res.Metadata, data.MetadataFilter{
...
func NewShareUri(id, password string) string {
	if password != "" {
		return fmt.Sprintf("%s://%s:%s@%s", constants.CloudScheme, id, password, constants.FileSystemShare)
	}
	return fmt.Sprintf("%s://%s@%s", constants.CloudScheme, id, constants.FileSystemShare)
}

func NewMyUri(id string) string {
	if id == "" {
		return fmt.Sprintf("%s://%s", constants.CloudScheme, constants.FileSystemMy)
	}
	return fmt.Sprintf("%s://%s@%s", constants.CloudScheme, id, constants.FileSystemMy)
}
