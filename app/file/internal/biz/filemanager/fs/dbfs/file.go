...
type (
	File struct {
		...
		FileFolderSummary *filepb.FolderSummary
...
// If isRoot is true, the URI will be returned from ownerId's view.
// Otherwise, the URI will be returned from userId's view.
...
// UserRoot return the root files from userId's view.
...
// Root return the root files from ownerId's view.
...
// RootUri return the URI of the userId root files under ownerId's view.
...
func (f *File) Replace(model *ent.File) *File {
	...
	if f.IsRootFile() {
		// If target is a root files, the userId path should remain the same.
...
// Ancestors return all ancestors of the files, until the ownerId root is reached.
func (f *File) Ancestors() []*File {
	return f.AncestorsChain()[1:]
}

// AncestorsChain return all ancestors of the files (including itself), until the ownerId root is reached.
...
// IsRootFolder return true if the files is the root folder under userId's view.
func (f *File) IsRootFolder() bool {
	return f.Type() == types.FileTypeFolder && f.IsRootFile()
}

// IsRootFile return true if the files is the root files under userId's view.
func (f *File) IsRootFile() bool {
	uri := f.Uri(false)
	p := uri.Path()
	return f.Model.Name == data.RootFolderName || p == fs.Separator || p == ""
}
...
func (f *File) PrimaryEntity() fs.Entity {
	...
	return fs.NewEmptyEntity(f.OwnerID())
}
...
func (f *File) FolderSummary() *filepb.FolderSummary {
