...
var (
	...
	ErrPermissionDenied = commonpb.ErrorForbidden("Permission denied")

	ErrShareIncorrectPassword  = userpb.ErrorInvalidPassword("Incorrect share password")
	ErrFileCountLimitedReached = pb.ErrorFileCountLimitReached("Walked files count reached limit")
	ErrSymbolicFolderFound     = commonpb.ErrorForbidden("Symbolic folder cannot be walked into")
	ErrLoginRequired           = errors.Unauthorized("Login required", "Login required")
...
type (
	// Navigator is a navigator for database files system.
	Navigator interface {
		Recycle()
		// To returns the files by path. If given path is not exist, returns ErrFileNotFound and most-recent ancestor.
		To(ctx context.Context, path *fs.URI) (*File, error)
		// Children returns the children of the parent files.
		...
		// Walk walks the files tree until limit is reached.
	...
	ListArgs            struct {
		...
		Search         *data.SearchFileParameters
		SharedWithMe   bool
...
func (b *baseNavigator) walkNext(ctx context.Context, root *File, next string, isLeaf bool) (*File, error) {
	...
	child, err := b.fileClient.GetChildFile(ctx, model, b.userId, next, isLeaf)
	if err != nil {
		if ent.IsNotFound(err) {
			return nil, fs.ErrPathNotExist.WithCause(err)
		}
...
func (b *baseNavigator) children(ctx context.Context, parent *File, args *ListArgs) (*ListResult, error) {
	...
	children, err := b.fileClient.GetChildFiles(ctx, &data.ListFileParameters{
...
func (b *baseNavigator) search(ctx context.Context, parent *File, args *ListArgs) (*ListResult, error) {
	...
	stepLevel := func(level int) (bool, error) {
		...
		listCtx := context.WithValue(ctx, data.LoadFilePublicMetadata{}, nil)
		for walkedFolder <= b.config.MaxRecursiveSearchedFolder {
			...
			parents = append(parents, lo.SliceToMap(
				lo.FilterMap(res.Files, func(model *ent.File, index int) (*File, bool) {
					p := parents[level][model.FileParentID]
					f := newFile(p, model)
	...
	for len(res) < originalPageSize && walkedFolder <= b.config.MaxRecursiveSearchedFolder {
		...
		newRes := lo.FilterMap(searchRes.Files, func(model *ent.File, index int) (*File, bool) {
			p := parents[startLevel][model.FileParentID]
			f := newFile(p, model)
			f.Path[pathIndexUser] = p.Uri(false).Join(model.Name)
			return b.listFilter(ctx, f)
		})
	...
	searchRes := &ListResult{
		Files:                 res,
		MixedType:             true,
		Pagination:            &pb.PaginationResults{IsCursor: true},
...
func newMyUri() *fs.URI {
	res, _ := fs.NewUriFromString(constants.CloudScheme + "://" + string(constants.FileSystemMy))
	return res
}

func newMyIDUri(uid string) *fs.URI {
	res, _ := fs.NewUriFromString(fmt.Sprintf("%s://%s@%s", constants.CloudScheme, uid, constants.FileSystemMy))
	return res
}

func newTrashUri(name string) *fs.URI {
	res, _ := fs.NewUriFromString(fmt.Sprintf("%s://%s", constants.CloudScheme, constants.FileSystemTrash))
	return res.Join(name)
}

func newSharedWithMeUri(id string) *fs.URI {
	res, _ := fs.NewUriFromString(fmt.Sprintf("%s://%s", constants.CloudScheme, constants.FileSystemSharedWithMe))
	return res.Join(id)
}
