...
func (f *DBFS) PrepareUpload(ctx context.Context, req *fs.UploadRequest, opts ...fs.Option) (*fs.UploadSession, error) {
	...
	// Get most recent ancestor or target files
	...
	// If files already exist, and update operation is suspended or existing files is not a files
	if fileExisted && (req.Props.EntityType == nil || ancestor.Type() != types.FileTypeFile) {
		return nil, fs.ErrFileExisted
	}

	// If files not exist, only empty entity / version entity is allowed
	...
	fc, dbTx, ctx, err := data.WithTx(ctx, f.fileClient)
	if err != nil {
	...
	if err := data.CommitWithStorageDiff(ctx, dbTx, f.l, f.userClient); err != nil {
		return nil, serializer.NewError(serializer.CodeDBError, "Failed to commit files upload preparation", err)
	}
...
func (f *DBFS) CompleteUpload(ctx context.Context, session *fs.UploadSession) (fs.File, error) {
	...
	// Confirm locks on placeholder files
	...
	ownerID := filePrivate.OwnerID()
	// Max allowed versions
	maxVersions := 1
	//if entityType == types.EntityTypeVersion &&
	//	ownerID.Settings.VersionRetention &&
	//	(len(ownerID.Settings.VersionRetentionExt) == 0 || util.IsInExtensionList(ownerID.Settings.VersionRetentionExt, files.Name())) {
	//	// Retention is enabled for this files
	//	maxVersions = ownerID.Settings.VersionRetentionMax
	//	if maxVersions == 0 {
	//		// Unlimited versions
	...
	// Start transaction to update files
	...
	if err != nil {
		_ = data.Rollback(tx)
		return nil, serializer.NewError(serializer.CodeDBError, "Failed to update placeholder files", err)
	}
	...
	diff, err := fc.CapEntities(ctx, filePrivate.Model, int(ownerID), maxVersions, entityType)
	if err != nil {
		_ = data.Rollback(tx)
	...
	if entityType == types.EntityTypeVersion {
		...
		diff, err = fc.CapEntities(ctx, filePrivate.Model, int(ownerID), 0, types.EntityTypeThumbnail)
		if err != nil {
			_ = data.Rollback(tx)
	...
	if err := data.CommitWithStorageDiff(ctx, tx, f.l, f.userClient); err != nil {
		return nil, serializer.NewError(serializer.CodeDBError, "Failed to commit files change", err)
	}
	...
	file, err = f.Get(ctx, session.Props.Uri, WithFileEntities(), WithNotRoot())
	if err != nil {
		return nil, fmt.Errorf("failed to get updated files: %w", err)
	}
...
func (f *DBFS) CancelUploadSession(ctx context.Context, path *fs.URI, sessionID string, session *fs.UploadSession) ([]fs.Entity, *fs.IndexDiff, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to get placeholder files: %w", err)
	}
	...
	if err := f.fileClient.RemoveMetadata(ctx, filePrivate.Model, MetadataUploadSessionID, ThumbDisabledKey); err != nil {
		return nil, nil, fmt.Errorf("failed to remove upload session metadata: %w", err)
	}
	...
	if session != nil && session.LockToken != "" {
		defer func() {
			if err := f.ls.Unlock(time.Now(), session.LockToken); err != nil {
				f.l.WithContext(ctx).Warnf("Failed to unlock files %q: %s", filePrivate.Uri(true).String(), err)
			}
		}()
	}

	if len(filePrivate.Entities()) == 1 {
		// Only one placeholder entity, just delete this files
