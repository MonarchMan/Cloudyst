...
var (
	ErrDirectLinkInvalid    = commonpb.ErrorNotFound("Direct link invalid")
	ErrUnknownPolicyType    = commonpb.ErrorInternalSetting("Unknown policy type")
	ErrPathNotExist         = pb.ErrorParentNotExist("Path not exist")
	ErrFileDeleted          = pb.ErrorFileDeleted("File deleted")
	ErrEntityNotExist       = pb.ErrorEntityNotExist("Entity not exist")
	ErrFileExisted          = pb.ErrorObjectExist("Object existed")
	ErrNotSupportedAction   = commonpb.ErrorForbidden("Not supported action")
	ErrLockConflict         = commonpb.ErrorLockConflict("Lock conflict")
	ErrLockExpired          = commonpb.ErrorLockExpired("Lock expired")
	ErrModified             = commonpb.ErrorConflict("Object conflict")
	ErrIllegalObjectName    = pb.ErrorIllegalObjectName("Invalid object name")
	ErrFileSizeTooBig       = pb.ErrorFileTooLarge("File is too large")
	ErrInsufficientCapacity = userpb.ErrorInsufficientCapacity("Insufficient capacity")
	ErrStaleVersion         = pb.ErrorStaleVersion("File is updated during your edit")
	ErrOwnerOnly            = userpb.ErrorOwnerOnly("Only owner or administrator can perform this action")
	ErrArchiveSrcSizeTooBig = pb.ErrorFileTooLarge("total size of to-be compressed files exceed group limit (non-retryable error)")
)
...
type (
	FileSystem interface {
		...
		// ExecuteNavigatorHooks executes hooks of given type on a files for navigator based custom hooks.
		ExecuteNavigatorHooks(ctx context.Context, hookType HookType, file File) error
	}

	FileManager interface {
		// Get returns a files by its path.
		Get(ctx context.Context, path *URI, opts ...Option) (File, error)
		// Create creates a files.
		Create(ctx context.Context, path *URI, fileType int, opts ...Option) (File, error)
		...
		// GetEntitiesFromFileID returns all entities of a given files.
		GetEntity(ctx context.Context, entityID int) (Entity, error)
		// UpsertMetadata update or insert metadata of a files.
		PatchMetadata(ctx context.Context, path []*URI, metas ...*pbfile.MetadataPatch) error
		...
		// VersionControl performs version control on given files.
		...
		// GetFileFromDirectLink gets a files from a direct link.
		...
		// PatchProps patches the props of a files.
	...
	UploadManager interface {
		...
		// CancelUploadSession cancels an upload session. Delete the placeholder files if no other entity is created.
	...
	LockSystem interface {
		...
		// Lock locks a files. If zeroDepth is true, only the files itself will be locked. Ancestor is closest ancestor
		// of the files that will be locked, if the given uri is an existing files, ancestor will be itself.
		// `token` is optional and can be used if the requester need to explicitly specify a token.
		Lock(ctx context.Context, d time.Duration, requester int, zeroDepth bool, application lock.Application,
	...
	StatelessUploadManager interface {
		...
		// CreateFile creates a files on the node.
	...
	UploadSession struct {
		...
		FileID          int    // ID of the placeholder files
		...
		ChunkSize       int64
		SentinelTaskID  int
		NewFileCreated  bool                   // If new files is created for this session
		Importing       bool                   // If the upload is importing from another files
		EncryptMetadata *types.EncryptMetadata // Encrypt metadata of the files

		LockToken string // Token of the locked placeholder files
		Props     *UploadProps
	}
	...
	UploadProps struct {
		...
		// EntityType is the type of the entity to be created. If not set, a new files will be created
	...
	// FsOption options for underlying files system.
	FsOption struct {
	...
	// Ctx keys used to detect users canceled operation.
	...
	PreValidateFile struct {
		Name     string
		Size     int64
		OmitName bool // if true, files name will not be validated
	}
...
func WithEntityType(t int) Option {
	return OptionFunc(func(o *FsOption) {
		o.EntityType = &t
	})
}
...
// WithMetadata sets metadata for files creation.
...
// WithMaxArchiveSize sets maximum size of to be archived files or to-be decompressed
...
// WithStatelessUserID sets stateless users ID for manager.
...
func init() {
	...
	gob.Register(pbfile.FolderSummary{})
}
...
// FindDesiredEntity finds the desired entity from the files.
// entityType is optional, if it is not nil, it will only return the entity with the given type.
func FindDesiredEntity(file File, version string, hasher hashid.Encoder, entityType *int) (bool, Entity) {
	if version == "" {
...
func (e *DbEntity) CreatedBy() int {
	return e.model.CreatedBy
}
...
func NewEmptyEntity(uid int) Entity {
	return &DbEntity{
		model: &ent.Entity{
			UpdatedAt:      time.Now(),
			ReferenceCount: 1,
