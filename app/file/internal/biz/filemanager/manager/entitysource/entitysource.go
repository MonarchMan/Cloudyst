...
type EntitySource interface {
	...
	IsLocal() bool
	// LocalPath returns the local path of the source files.
	LocalPath(ctx context.Context) string
	// Apply applies the options to the source.
	Apply(opts ...EntitySourceOption)
	// CloneToLocalSrc clones the source to a local files source.
	CloneToLocalSrc(t int, src string) (EntitySource, error)
	// ShouldInternalProxy returns true if the source will/should be proxied by external proxy.
...
// WithSpeedLimit set speed limit for files source (if supported)
...
// WithExpire set expire time for files source
...
// WithDownload set files URL as download
...
// WithNoInternalProxy overwrite policy's external proxy setting
...
// WithDisplayName set display name for files source
...
// WithContext set context for files source
...
func (f *entitySource) CloneToLocalSrc(t int, src string) (EntitySource, error) {
	...
	handler := local.New(policy, f.l.Logger(), f.config)
...
func (f *entitySource) Serve(w http.ResponseWriter, r *http.Request, opts ...EntitySourceOption) {
	...
	if f.IsLocal() {
		// For local files, validate files existence by resetting rsc
		if err := f.resetRequest(); err != nil {
			f.l.WithContext(r.Context()).Warnf("Failed to serve local entity %q: %s", err, f.e.Source())
			http.Error(w, "Entity constants does not exist.", http.StatusNotFound)
			return
		}
	}
	...
	if !f.IsLocal() {
		// for non-local files, reverse-proxy the request
		...
		proxy := &httputil.ReverseProxy{
			...
			ErrorHandler: func(writer http.ResponseWriter, request *http.Request, err error) {
				f.l.WithContext(r.Context()).Errorf("Reverse proxy error in %q: %s", request.URL.String(), err)
	...
	code := http.StatusOK
	// If Content-Type isn't set, use the files's extension to find it, but
	...
	if !haveType {
		...
		if ctype == "" {
			...
			_, err := f.Seek(0, io.SeekStart) // rewind to output whole files
			if err != nil {
	...
	case errNoOverlap:
		if size == 0 {
			...
			// limit the size of the response. If the files is empty,
	...
	if sumRangesSize(ranges) > size {
		...
		// is larger than the size of the files by
...
func (f *entitySource) Url(ctx context.Context, opts ...EntitySourceOption) (*EntityUrl, error) {
	...
	if f.ShouldInternalProxy() {
		...
		srcUrl, err = auth.SignURI(f.generalAuth, base.String(), expire)
		if err != nil {
			return nil, fmt.Errorf("failed to sign external proxy URL: %w", err)
		}

		if f.IsLocal() {
			// For local files, we need to apply proxy if needed
...
func (f *entitySource) getRsc(pos int64) (io.ReadCloser, error) {
	...
	if f.IsLocal() {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to open inbound files: %w", err)
		}

		if pos > 0 {
			_, err = file.Seek(pos, io.SeekStart)
			if err != nil {
				return nil, fmt.Errorf("failed to seek inbound files: %w", err)
			}
		}
...
func parseRange(s string, size int64) ([]httpRange, error) {
	...
	for _, ra := range strings.Split(s[len(b):], ",") {
		...
		if start == "" {
			...
			// range start relative to the end of the files,
			...
			r.start = i
			if end == "" {
				// If no end is specified, range extends to end of the files.
				r.length = size - r.start
			} else {
				i, err := strconv.ParseInt(end, 10, 64)
