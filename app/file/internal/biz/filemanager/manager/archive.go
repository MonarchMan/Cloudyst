...
func (m *manager) ListArchiveFiles(ctx context.Context, uri *fs.URI, entity, zipEncoding string) ([]*pbfile.ArchivedFile, error) {
	...
	if file.Type() != types.FileTypeFile {
		return nil, fs.ErrNotSupportedAction.WithCause(fmt.Errorf("path %s is not a files", uri))
	}
	...
	if zipEncoding != "" {
		...
		if !ok {
			return nil, fs.ErrNotSupportedAction.WithCause(fmt.Errorf("not supported zip encoding: %s", zipEncoding))
		}
	}
	...
	if found {
		return res.([]*pbfile.ArchivedFile), nil
	}
	...
	var readerFunc func(ctx context.Context, file io.ReaderAt, size int64, textEncoding encoding.Encoding) ([]*pbfile.ArchivedFile, error)
	switch file.Ext() {
	case "zip":
	...
	default:
		return nil, fs.ErrNotSupportedAction.WithCause(fmt.Errorf("not supported archive format: %s", file.Ext()))
	}
...
func (m *manager) CreateArchive(ctx context.Context, uris []*fs.URI, writer io.Writer, opts ...fs.Option) (int, error) {
	...
	for _, uri := range uris {
		...
		if err != nil {
			return 0, fmt.Errorf("failed to get files %s: %w", uri, err)
		}
	...
	for _, file := range files {
		if file.Type() == types.FileTypeFile {
			if err := m.compressFileToArchive(ctx, "/", file, zipWriter, o.ArchiveCompression, o.DryRun); err != nil {
				failed++
				m.l.WithContext(ctx).Warnf("Failed to compress files %s: %s, skipping it...", file.Uri(false), err)
			}
			...
			if err := m.Walk(ctx, file.Uri(false), intsets.MaxInt, func(f fs.File, level int) error {
				...
				if err := m.compressFileToArchive(ctx, strings.TrimPrefix(f.Uri(false).Dir(),
					...
					failed++
					m.l.WithContext(ctx).Warnf("Failed to compress files %s: %s, skipping it...", f.Uri(false), err)
				}
				...
				m.l.WithContext(ctx).Warnf("Failed to walk folder %s: %s, skipping it...", file.Uri(false), err)
				failed++
			}
		}
	}
...
func (m *manager) compressFileToArchive(ctx context.Context, parent string, file fs.File, zipWriter *zip.Writer,
	...
	if err != nil {
		return fmt.Errorf("failed to get entity source for files %s: %w", file.Uri(false), err)
	}
	...
	m.l.WithContext(ctx).Debugf("Compressing %s to archive...", file.Uri(false))
...
func getZipFileList(ctx context.Context, file io.ReaderAt, size int64, textEncoding encoding.Encoding) ([]*pbfile.ArchivedFile, error) {
	zr, err := zip.NewReader(file, size)
	if err != nil {
		return nil, fmt.Errorf("failed to create zip reader: %w", err)
	}

	fileList := make([]*pbfile.ArchivedFile, 0, len(zr.File))
	for _, f := range zr.File {
		...
		fileList = append(fileList, &pbfile.ArchivedFile{
...
func get7zFileList(ctx context.Context, file io.ReaderAt, size int64, extEncoding encoding.Encoding) ([]*pbfile.ArchivedFile, error) {
