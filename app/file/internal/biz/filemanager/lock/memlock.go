...
type LockDetails struct {
	...
	ZeroDepth bool
	// FileType is the type of the files being locked. This is used to display users-friendly error message.
	Type int
...
func NewMemLS(hasher hashid.Encoder, l log.Logger) LockSystem {
	return &memLS{
		byName:  make(map[string]map[string]*memLSNode),
		byToken: make(map[string]*memLSNode),
		hasher:  hasher,
		l:       log.NewHelper(l, log.WithMessageKey("memlock")),
	}
}
...
func (m *memLS) Confirm(now time.Time, request LockInfo) (func(), string, error) {
	...
	m.l.Debugf("Memlock confirm: NS:%s, Root: %s, Token: %v", request.Ns, request.Root, request.Token)
...
func (m *memLS) Refresh(now time.Time, duration time.Duration, token string) (LockDetails, error) {
	...
	m.l.Debugf("Memlock refresh: Token: %s, Duration: %v", token, duration)
	n := m.byToken[token]
	if n == nil {
...
func (m *memLS) Create(now time.Time, details ...LockDetails) ([]string, error) {
	...
	for i, detail := range details {
		...
		m.l.Debugf("Memlock create: NS:%s, Root: %s, Duration: %v, ZeroDepth: %v", detail.Ns, detail.Root, detail.Duration, detail.ZeroDepth)
...
type memLSNode struct {
	...
	// This is used to find out who is locking this files.
