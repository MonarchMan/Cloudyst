...
func NewMusicCoverGenerator(l log.Logger, settings setting.Provider) *MusicCoverGenerator {
	return &MusicCoverGenerator{l: log.NewHelper(l, log.WithMessageKey("biz-thumb")), settings: settings}
}
...
func (v *MusicCoverGenerator) Generate(ctx context.Context, es entitysource.EntitySource, ext string, previous *Result) (*Result, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("faield to read audio tags from files: %w", err)
	}

	p := m.Picture()
	if p == nil || len(p.Data) == 0 {
		return nil, fmt.Errorf("no cover found in given files")
	}
	...
	if _, err := thumbFile.Write(p.Data); err != nil {
		return &Result{Path: tempPath}, fmt.Errorf("failed to write cover to files: %w", err)
	}
