...
func NewLibRawGenerator(l log.Logger, settings setting.Provider) *LibRawGenerator {
	return &LibRawGenerator{l: log.NewHelper(l, log.WithMessageKey("biz-thumb")), settings: settings}
}
...
func (l *LibRawGenerator) Generate(ctx context.Context, es entitysource.EntitySource, ext string, previous *Result) (*Result, error) {
	...
	if err := cmd.Run(); err != nil {
		l.l.WithContext(ctx).Warnf("Failed to invoke dcraw: %s", dcrawErr.String())
