...
func MasterSlaveCallbackUrl(base *url.URL, driver, id, secret string) *url.URL {
	apiBaseURI, _ := url.Parse(path.Join(constants.FileAPIPrefix+"/callback", driver, id, secret))
...
func MasterArchiveDownloadUrl(base *url.URL, sessionID string) *url.URL {
	routes, err := url.Parse(path.Join(constants.FileAPIPrefix, "file", "archive", sessionID, "archive.zip"))
	if err != nil {
		return nil
	}
...
func MasterFileContentUrl(base *url.URL, entityId, name string, download, thumb bool, speed int64) *url.URL {
	...
	route, _ := url.Parse(constants.FileAPIPrefix + fmt.Sprintf("/file/content/%s/%d/%s", entityId, speed, name))
	if base != nil {
...
func MasterWopiSrc(base *url.URL, sessionId string) *url.URL {
	route, _ := url.Parse(constants.FileAPIPrefix + "/file/wopi/" + sessionId)
	return base.ResolveReference(route)
}
