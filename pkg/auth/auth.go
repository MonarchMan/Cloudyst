...
func SignRequest(instance Auth, r *http.Request, expires *time.Time) *http.Request {
	// 处理有效期
	expireTime := int64(0)
	if expires != nil {
	...
	// 生成签名
	sign := instance.Sign(getSignContent(r), expireTime)

	// 将签名加到请求Header中
	r.Header["Authorization"] = []string{constants.TokenHeaderPrefixCr + sign}
	return r
}
...
func CheckRequest(instance Auth, r *http.Request) error {
	...
	sign[0] = strings.TrimPrefix(sign[0], constants.TokenHeaderPrefixCr)

	return instance.Check(getSignContent(r), sign[0])
}

func isUploadDataRequest(r *http.Request) bool {
	return strings.Contains(r.URL.Path, constants.FileAPIPrefix+"/slave/upload/") && r.Method != http.MethodPut
...
func getSignContent(r *http.Request) (rawSignString string) {
	...
	// 读取所有待签名Header
	rawSignString = serializer.NewRequestSignString(getUrlSignContent(r.URL), strings.Join(signedHeader, "&"), string(body))

	return rawSignString
}

// SignURI 对URI进行签名
func SignURI(instance Auth, uri string, expires *time.Time) (*url.URL, error) {
	// 处理有效期
	expireTime := int64(0)
	if expires != nil {
		expireTime = expires.Unix()
	...
	// 生成签名
	sign := instance.Sign(getUrlSignContent(base), expireTime)
...
func CheckURI(instance Auth, url *url.URL) error {
	...
	return instance.Check(getUrlSignContent(url), sign)
}
...
func getUrlSignContent(url *url.URL) string {
	// host := url.Host
	// if host == "" {
	...
	return strings.TrimPrefix(url.Path, constants.FileAPIPrefix)
}
