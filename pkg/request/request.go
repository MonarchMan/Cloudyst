...
func NewClient(mode string, opts ...Option) Client {
	client := &HTTPClient{
...
func (c *HTTPClient) Request(method, target string, body io.Reader, opts ...Option) *Response {
	...
	req.Header.Set("User-Agent", "CloudDyst/"+constants.BackendVersion)

	if options.ctx != nil && options.withCorrelationID {
		req.Header.Add(CorrelationHeader, util.TraceID(options.ctx))
	}

	mode := c.mode
	if options.masterMeta && mode == constants.MasterMode {
	...
	if options.sign != nil {
		...
		switch method {
		case "PUT", "POST", "PATCH":
			auth.SignRequest(options.sign, req, &expire)
		default:
			if resURL, err := auth.SignURI(options.sign, req.URL.String(), &expire); err == nil {
				req.URL = resURL
			}
		}
	}
	...
	return &Response{Err: nil, Response: resp, l: log.NewHelper(options.logger, log.WithMessageKey("response"))}
}
...
func (resp *Response) DecodeResponse() (*serializer.Response, error) {
	...
	if err != nil {
		if resp.l != nil {
			resp.l.Debugf("Failed to parse response: %s", respString)
		}
