...
// NewMemoStore 新建内存存储
func NewMemoStore(persistFile string, l log.Logger) *MemoStore {
	store := &MemoStore{
		Store: &sync.Map{},
	}
	...
	if persistFile != "" {
		if err := store.Restore(persistFile); err != nil {
			l.Log(log.LevelWarn, "Failed to restore cache from disk: %s", err)
		}
	}
...
// Restore memory cache from disk files
func (store *MemoStore) Restore(path string) error {
	...
	f, err := os.Open(path)
	if err != nil {
		return fmt.Errorf("failed to read cache files: %s", err)
	}
	...
	if err := dec.Decode(&persisted); err != nil {
		return fmt.Errorf("unknown cache files format: %s", err)
	}
