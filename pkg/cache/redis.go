...
func NewRedisStore(l log.Logger, size int, db int, network string, addr string, username string, password string,
	useTls bool, tlsSkipVerify bool) *RedisStore {
	return &RedisStore{
		pool: &redis.Pool{
			MaxIdle:     size,
			...
			Dial: func() (redis.Conn, error) {
				...
				if err != nil {
					l.Log(log.LevelFatal, "Failed to create Redis connection: %s", err)
				}
				return c, nil
			},
		},
